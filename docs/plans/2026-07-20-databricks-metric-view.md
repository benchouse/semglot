...
**Architecture:** A new `Emitter`/`Configurable` in `layer/`, parallel to `snowflake_semantic_view.go`. It walks the IR: each table with ≥1 metric becomes a metric view rooted at that table (`source`), with direct IR relationships as `joins`, dimensions (own + joined) as `fields`, and metrics (lowered via the existing `renderSQL`) as `measures`. Cross-grain and window/conversion metrics degrade to a `comment` note. Output is raw YAML via `yaml.v3`.

**Tech Stack:** Go, `gopkg.in/yaml.v3`, existing `ir` + `layer` packages.

## Global Constraints

- Package `layer` (not the old `dialect`); this branch is based on `feature/sv-view-schema-option`.
- Target-type name is exactly `databricks-metric-view`.
...
**Files:**
- Create: `layer/databricks_metric_view.go`
- Test: `layer/databricks_metric_view_test.go`

**Interfaces:**
- Consumes: `ir.Model`, `ir.Table`, `ir.Field`, `ir.Measure`, `ir.Metric`, `ir.Relationship`, `ir.Expr` (and AST nodes `ir.Ref`, `ir.Binary`, `ir.Agg`, `ir.Window`, `ir.Conversion`); `metricResolver`, `renderSQL`, `enumClause`, `appendClause`; `layer.Register`, `layer.Options`, `layer.Emitter`, `layer.Configurable`.
- Produces: a registered layer named `databricks-metric-view` implementing `Emitter` + `Configurable`. Emits `<lowercased-table-name>.yaml` per fact table into the output dir.

- [ ] **Step 1: Write the failing unit test**

Create `layer/databricks_metric_view_test.go`:
...
Run: `go test ./layer/ -run TestDatabricksMetricView 2>&1 | tail -20`
...
Run: `go test ./layer/ -run TestDatabricksMetricView -v 2>&1 | tail -25`
...
- [ ] **Step 5: Run the full layer package to check nothing else broke**

Run: `go test ./layer/ 2>&1 | tail -5`
Expected: `ok  github.com/benchouse/semglot/layer`

- [ ] **Step 6: Commit**

```bash
git add layer/databricks_metric_view.go layer/databricks_metric_view_test.go
...
**Files:**
- Modify: `cmd/semglot/main.go` (the `snowflakeTargets` var ~line 38 and its use ~line 93)

**Interfaces:**
- Consumes: the registered `databricks-metric-view` layer from Task 1.
- Produces: `--target-type databricks-metric-view` requires `--database`/`--config`; the target auto-appears in `layer.Names()` usage output.

- [ ] **Step 1: Write the failing test**

Add to `cmd/semglot/main_test.go` (append; keep existing package clause):
...
Run: `go test ./cmd/semglot/ -run TestBuildDatabricksRequiresDatabase -v 2>&1 | tail -15`
Expected: FAIL — the target is not yet in the required-catalog set, so `buildCmd` returns 0 (emits unqualified output).

- [ ] **Step 3: Add the target to the required-catalog set**

In `cmd/semglot/main.go`, replace the `snowflakeTargets` declaration:
...
Then update the single use site (in `buildCmd`) from:
...
```bash
git add cmd/semglot/main.go cmd/semglot/main_test.go
...
so building without --database fails fast instead of emitting an
...
**Interfaces:**
- Consumes: `sourceDirs` (already defined in the test package), `layer.AsEmitter`, `layer.AsParser`, `layer.Configurable`, `layer.Options`.
...
Note: `os`, `path/filepath`, `sort`, `strings`, and `layer` are already imported in `test/integration_test.go` (verify; add any missing to the import block).
...
Expected: all packages `ok` (`cmd/semglot`, `layer`, `test`; `ir` has no tests).
...
/tmp/semglot build --source test/models/ecommerce/dbt/semantic --source test/models/ecommerce/dbt/marts \
  --target /tmp/dbx-out --target-type databricks-metric-view --database ANALYTICS && \
ls /tmp/dbx-out && head -20 /tmp/dbx-out/fct_orders.yaml
```
Expected: prints `wrote to /tmp/dbx-out (dbt -> databricks-metric-view)`, lists the fact yaml files, and shows a valid metric-view header (`version: "1.1"`, `source: analytics.main.fct_orders`).
